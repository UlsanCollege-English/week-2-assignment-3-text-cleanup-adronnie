...
def top_k_frequent_first_tie(words: List[str], k: int) -> List[str]:
    """Return up to k words ordered by frequency (high to low). Ties: first appearance wins."""
    if k <= 0:
        raise ValueError("k must be positive")

    freq = Counter(words)

    # Track the index of first appearance
    first_seen = {}
    for idx, w in enumerate(words):
        if w not in first_seen:
            first_seen[w] = idx

    # Sort by: frequency desc, first appearance asc
    sorted_words = sorted(
        freq.keys(),
